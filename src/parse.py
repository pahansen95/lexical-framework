"""
RDP Framework - Recursive Descent Parser Framework
...
class Parser:
  ...
  def __init__(
    self, tokens: TokenStream, structural_rules: Type[StructuralRules] = StructuralRules, debug: bool = False
  ):
    self.tokens = tokens
    self.debug = debug
  ...
  @contextmanager
  def structural_context(self, rules: Type[StructuralRules], active: bool = True, capture: bool = True):
  ...
  def repeats(self, parser_fn: Callable, min_count: int = 0, max_count: Optional[int] = None) -> List:
    """
  ...
  *,
  ...
  def decorator(func):
    ...
    def wrapper(self, *args, **kwargs):
      ...
      ctx = RuleContext(rule_name, self.tokens.current_pos(), capture=capture, structural=structural)
